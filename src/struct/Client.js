    ...
    constructor(options) {
        ...
        this.presence = {
            ...
            next: async (iteration = 1) => {
                ...
                await this.user.setActivity(`${this.presence.activities[nextIndex].title} | ${this.prefix}help`, { url: "https://shodanbot.com", type: this.presence.activities[nextIndex].type });
            },
            fix: async () => {
                let current = { type: this.presence.activities.find(a => { return a.title == this.user.presence.game.name }).type, title: this.user.presence.game.name };
