module.exports = {
    ...
    execute(message, args, client, logger) {
        ...
        client.sql.query(`SELECT * FROM tags WHERE tagName=${args[0].replace(/"/g, "")}`, (error, rows, fields) => {
            if(!message.author.id == rows[0].authorID && !client.config.bot.admins.includes(message.author.id)) return message.channel.send("You do not have the permissions to delete that tag.");
